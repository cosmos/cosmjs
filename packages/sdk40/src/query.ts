...
async function proveByKey(client: Client, store: string, key: Uint8Array): Promise<Uint8Array> {
...
export async function proveAccount(client: Client, address: string): Promise<google.protobuf.Any> {
  ...
  const response = await proveByKey(client, "acc", accountKey);
...
export async function proveBalance(client: Client, address: string, denom: string): Promise<Coin> {
  const binAddress = Bech32.decode(address).data;
  ...
  const response = await proveByKey(client, "bank", bankKey);
