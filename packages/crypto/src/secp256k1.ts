...
export class Secp256k1 {
  public static async makeKeypair(privkey: Uint8Array): Promise<Secp256k1Keypair> {
    ...
    const out: Keypair = {
      ...
      pubkey: Encoding.fromHex(keypair.getPublic(false, "hex")),
    };
  ...
  public static async createSignature(
    ...
    if (typeof recoveryParam !== "number") throw new Error("Missing recovery param");
    return new ExtendedSecp256k1Signature(
      Uint8Array.from(signature.r.toArray()),
      Uint8Array.from(signature.s.toArray()),
      recoveryParam,
    );
  }
