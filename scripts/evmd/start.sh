...
TMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/evmd.XXXXXXXXX")
chmod 777 "$TMP_DIR"
echo "Using temporary dir $TMP_DIR"
EVMD_LOGFILE="$TMP_DIR/evmd.log"

# Use a fresh volume for every start
docker volume rm -f evmd_data
...
docker run --rm \
  ...
  --mount type=volume,source=evmd_data,target=/root \
  "$REPOSITORY:$VERSION" \
  /template/run_evmd.sh \
  >"$EVMD_LOGFILE" 2>&1 &

echo "evmd running on http://localhost:$TENDERMINT_PORT_HOST and logging into $EVMD_LOGFILE"
