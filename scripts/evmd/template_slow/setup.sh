...
echo "Configuring timeouts (slow mode - 2x longer) ..."
CONFIG_TOML="$HOME/.evmd/config/config.toml"
"$gnused" -i.bak 's/timeout_propose = "[^"]*"/timeout_propose = "4s"/g' "$CONFIG_TOML"
"$gnused" -i.bak 's/timeout_propose_delta = "[^"]*"/timeout_propose_delta = "400ms"/g' "$CONFIG_TOML"
"$gnused" -i.bak 's/timeout_prevote = "[^"]*"/timeout_prevote = "1s"/g' "$CONFIG_TOML"
"$gnused" -i.bak 's/timeout_prevote_delta = "[^"]*"/timeout_prevote_delta = "400ms"/g' "$CONFIG_TOML"
"$gnused" -i.bak 's/timeout_precommit = "[^"]*"/timeout_precommit = "1s"/g' "$CONFIG_TOML"
"$gnused" -i.bak 's/timeout_precommit_delta = "[^"]*"/timeout_precommit_delta = "400ms"/g' "$CONFIG_TOML"
"$gnused" -i.bak 's/timeout_commit = "[^"]*"/timeout_commit = "2s"/g' "$CONFIG_TOML"
"$gnused" -i.bak 's/timeout_broadcast_tx_commit = "[^"]*"/timeout_broadcast_tx_commit = "10s"/g' "$CONFIG_TOML"
